...
func newInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(`
install registers obcmd-agent as a Windows service that starts
...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			s, err := m.CreateService(ServiceName, path, mgr.Config{
				DisplayName: "obcmd remote-exec agent",
				Description: "Polls the obcmd relay for encrypted commands.",
	...
	cmd.Flags().StringVar(&binPath, "bin-path", "", "explicit path to obcmd-agent.exe (default: this binary)")
	return cmd
}
