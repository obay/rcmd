...
func defaultConfigPath() string {
	...
	case "windows":
		...
		return filepath.Join(pd, "obcmd", "agent.toml")
	case "darwin":
		return "/usr/local/etc/obcmd/agent.toml"
	default:
		return "/etc/obcmd/agent.toml"
	}
}
...
func defaultLogPath() string {
	...
	case "windows":
		...
		return filepath.Join(pd, "obcmd", "agent.log")
	case "darwin":
		return "/usr/local/var/log/obcmd-agent.log"
	default:
		return "/var/log/obcmd-agent.log"
	}
}
...
Quick start:
  ...
  $EDITOR %s
  obcmd-agent run                  # run in foreground (good for first-time testing)

Windows service install:
  obcmd-agent install              # registers and starts the Windows service
...
func initConfig() error {
	...
	viper.SetEnvPrefix("OBCMD_AGENT")
...
EXAMPLES
  obcmd-agent version
  obcmd-agent version --json
  # -> {"kind":"version","name":"obcmd-agent","version":"v0.1.0",
...
EXAMPLES
  obcmd-agent keygen --count 2
  obcmd-agent keygen --count 2 --json
`),
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
